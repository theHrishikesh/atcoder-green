...
int main() 
{
    ...
    vvll cover(2005,vll(2005,0));
    rep(i,1,2001)
    {
        rep(j,1,2001)
        {
            cover[i][j] = grid[i][j] + cover[i - 1][j] + cover[i][j - 1] - cover[i - 1][j - 1];
        }
    }

    ll base = 0;

    vvll pref(2001,vll(2001,0));
    ...
    auto rect_sum = [&](ll u,ll d,ll l,ll r)
    {
        return pref[d][r]
            - pref[d][l - 1]
            - pref[u - 1][r]
    ...
    rep(i,0,n)
    {
        ll uni = rect_sum(u[i], d[i], l[i], r[i]);
        out(uni + base);
    }
}
