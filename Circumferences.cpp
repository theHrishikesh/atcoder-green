...
int main()
{
    ...
    auto euclid = [&](ll x1, ll y1, ll x2, ll y2)
    {
    ...
    rep(i,N)
    {
        rep(j,i + 1,N)
        {
            ll dsq = euclid(X[i], Y[i], X[j], Y[j]);
            ll llim = (R[i] - R[j]) * (R[i] - R[j]);
            ll ulim = (R[i] + R[j]) * (R[i] + R[j]);
            if (llim <= dsq && dsq <= ulim)
            {
                adj[i][j] = 1;
                adj[j][i] = 1;
            }
    ...
    rep(i,0,N)
    {
        if ((sx - X[i]) * (sx - X[i]) + (sy - Y[i]) * (sy - Y[i]) == R[i] * R[i]) start = i;
        if ((ex - X[i]) * (ex - X[i]) + (ey - Y[i]) * (ey - Y[i]) == R[i] * R[i]) stop = i;
    }
    vbl vis(N,0);
