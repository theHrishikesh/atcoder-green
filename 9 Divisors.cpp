...
{
    sll(N);
    vbl not_prime((ll)2e6 + 1,0);
    not_prime[1] = 1;
    for (ll i = 2; i < len(not_prime); i++)
    {
        if (not_prime[i]) continue;
        for (ll j = i + i; j < len(not_prime); j += i) not_prime[j] = 1;
    }

    vll primes;

    rep(i,2,len(not_prime)) if (!not_prime[i] && i <= N) primes.pb(i);
    ...
    rep(i,0,len(primes))
    {
        ...
        ll j = upper_bound(all(primes),limit) - primes.begin() - 1;
        if (j > i) ans += (j - i);
    }
    out(ans);
}
