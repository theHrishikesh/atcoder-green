...
int main()
{
    ...
    vvbl touched(n, vbl(m)), visited_stop(n, vbl(m));
    

    queue<array<ll,2>>q;
    q.push({1,1});
    visited_stop[1][1] = touched[1][1] = 1;
    ...
    while (len(q))
    {
        ...
        {
            ll nx = cx, ny = cy;
            while (grid[nx + dx4[i]][ny + dy4[i]] == '.')
            {
                nx += dx4[i];
                ny += dy4[i];
                touched[nx][ny] = 1;
            }
            if (!visited_stop[nx][ny])
            {
                visited_stop[nx][ny] = 1;
                q.push({nx, ny});
            }
        }
    }
    ...
    ll ans = 0;
    rep(i, n) rep(j, m) if (touched[i][j]) ans++;
    out(ans);
}
