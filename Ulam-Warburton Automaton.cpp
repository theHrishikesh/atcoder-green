...
{
    sll(h,w);
    vecs grid(h); rep(h) in(grid[i]);

    auto in_grid = [&](ll x, ll y) { return 0 <= x && x < h && 0 <= y && y < w;};

    auto valid_white = [&](ll cx, ll cy)
    {
        ll cnt = 0;
        rep(i,0,4)
        {
            ll dx = dx4[i], dy = dy4[i];
            ll nx = cx + dx;
            ll ny = cy + dy;
            if (in_grid(nx, ny) && grid[nx][ny] == '#') cnt++;
        }
        return cnt == 1;
    };


    queue<array<ll,2>> cq, nq;

    rep(i,h) rep(j,w)
    {
        if (grid[i][j] == '.' && valid_white(i,j))
        {
            cq.push({i,j});
        }
    }
    ...
    while (len(cq))
    {
        while (len(cq))
        {
            ...
            rep(i,0,4)
            {
                ll nx = cx + dx4[i];
                ll ny = cy + dy4[i];
                if (in_grid(nx,ny) && grid[nx][ny] == '.') nq.push({nx,ny});
            }
        ...
        while (len(nq))
        {
            auto [cx, cy] = nq.front(); nq.pop();
            if (grid[cx][cy] == '.' && valid_white(cx, cy)) cq.push({cx, cy});
        }
    }
