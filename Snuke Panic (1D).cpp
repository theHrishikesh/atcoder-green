...
{
    sll(n);
    vll t(n + 1,0), x(n + 1,0), a(n + 1,0);
    rep(i,1,n + 1)
    {
        in(t[i],x[i],a[i]);
    }
    vvll dp(n + 1,vll(5,NINF));
    ...
    rep(i,1,n + 1)
    {
        ...
        rep(curr,0,5)
        {
            rep(prev,0,5)
            {
                if (abs(curr - prev) <= delta && dp[i - 1][prev] != NINF)
                {
                    chmax(dp[i][curr],dp[i - 1][prev]);
                }
            }
            if (curr == x[i] && dp[i][curr] != NINF) dp[i][curr] += a[i];
        }
    }
