...
int main()
{
    ...
    rep(i,1,n)
    {
        ...
        while (currstop <= n - 1)
        {
            ...
            {
                ll term;
                if ((tottime - s[currstop])%f[currstop] == 0) term = ((tottime - s[currstop])/f[currstop]) + 1;
                else term = ((tottime - s[currstop])/f[currstop]) + 2;
                start = s[currstop] + ((term - 1) * f[currstop]);
            }
            tottime += start - tottime;
            tottime += c[currstop];
            currstop++;
        }
        out(tottime);
