...
int main()
{
    ...
    //dbg(t);
    ll mn = min(open, close);
    //dbg(mn);
    ll swaps = (mn + 1)/2;
    //dbg(swaps);
    ll forced = abs(open - close) / 2;
    //dbg(forced);
    ll swaprepcost = (swaps * a) + (forced * b);
    //dbg(swaprepcost);
    ll reponly = (open + close)/2;
    if ((open % 2 == 1) || (close % 2) == 1) reponly++;
    //dbg(reponly);
    ll reponlycost = reponly * b;
    ll cost = min(swaprepcost,reponlycost);
    out(cost);
}
