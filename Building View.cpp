...
int main()
{
    ...
    rep(i,0,n)
    {
        while (!nge.empty() && h[nge.top()] <= h[i]) nge.pop();
        if (!nge.empty()) lnge[i] = nge.top();
        else lnge[i] = -1;
        nge.push(i);
    }
    ...
    {
        sll(x);
        ll idx = lower_bound(all(score),x) - score.begin();
        out(n - idx);
    }
}
